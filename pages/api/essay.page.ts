...
export default async function handler(request: NextRequest) {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    const paragraphPrompt = `${convertParagraphToOutline(
      paragraph,
    )}\nConvert the above outline into a descriptive and extensive paragraph. Use specific examples to support arguments made. Use varied sentence structures and vocabulary. Refrain from using the same word or phrase more than once. Include any content surrounded by parentheses.\n`
