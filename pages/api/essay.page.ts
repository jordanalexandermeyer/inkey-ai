...
export default async function handler(request: NextRequest) {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    const paragraphPrompt = `${convertParagraphToOutline(
      paragraph,
    )}\nConvert the above outline into a descriptive and extensive paragraph. Use specific examples to support arguments made. Use varied sentence structures and vocabulary. Refrain from using the same word or phrase more than once.\n`
  ...
  prompts.push(
    `${convertArgsToOutline(
      args,
    )}\nThe contents of an essay are outlined above. The essay prompt is "${prompt}" and your thesis is, "${title}". Write a conclusion paragraph for the essay. Begin by restating your thesis. Next, broaden back out to a general topic. End with a closing statement. Write in paragraph form.\n`,
  )
