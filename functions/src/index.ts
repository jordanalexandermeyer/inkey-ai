...
const stripe = new Stripe(config.stripeSecretKey, {
  ...
  appInfo: {
    name: 'Inkey firestore-stripe-payments',
    version: '0.1.0',
  },
});
...
exports.createCheckoutSession = functions
  ...
  .firestore.document('/customers/{uid}/checkout_sessions/{id}')
      ...
      if (client === 'web') {
        ...
        const shippingCountries: Stripe.Checkout.SessionCreateParams.ShippingAddressCollection.AllowedCountry[] = collect_shipping_address
          ? (
              await admin
                .firestore()
                .collection(config.stripeConfigCollectionPath || 'products')
...
const manageSubscriptionStatusChange = async (
  ...
  const subscriptionData: Subscription = {
    ...
    product: admin.firestore().collection('products').doc(product.id),
    price: admin
      .firestore()
      .collection('products')
      .doc(product.id)
...
const deleteProductOrPrice = async (pr: Stripe.Product | Stripe.Price) => {
  if (pr.object === 'product') {
    await admin.firestore().collection('products').doc(pr.id).delete();
...
export const onUserDeleted = functions.auth.user().onDelete(async (user) => {
  ...
  const customer = (
    await admin.firestore().collection('customers').doc(user.uid).get()
  ).data();
